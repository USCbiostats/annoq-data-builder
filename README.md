...
- `Mapped_in_HRC` — `Y` if the hg19-equivalent SNP is in HRC r1.1, `N` if not found, `.` if
  `ref_hg19 != ref_hg38`.
- `HRC_chr_pos` — hg19 `chr:pos` when `Mapped_in_HRC = Y`, else `.` (HG19 info).
- `HRC_chr_pos_ref_alt` — hg19 `chr:posREF>ALT` (e.g. `18:10005A>T`) when `Mapped_in_HRC = Y`,
  else `.` (HG19 info).

> **Why `.` and not an empty string:** these are the last two columns of the row and are unset for
> every variant with `Mapped_in_HRC != 'Y'` (the majority). Writing `''` leaves the line ending in
> empty tab-separated fields, and Java's `String.split(regex)` **discards trailing empty fields** —
> so the Part-3 module counted 724 fields against a 726-column header, failed its field-count check
> in `Snp`'s constructor, and rejected every such variant with
> `Unable to create SNP information for ...` while `status.txt` grew by hundreds of MB per minute
> and the output stayed empty. Part 3 rewrites `.` → `""` during its cleanup pass, so the final
